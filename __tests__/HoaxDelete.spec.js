...
describe('Delete Hoax', () => {
    ...
    it("returns error body with 'You are not authorized to delete this hoax' for unauthorized request", async () => {
        const nowInMillis = Date.now();
        const response = await deleteHoax(5);
        expect(response.body.path).toBe('/api/1.0/hoaxes/5');
        expect(response.body.timestamp).toBeGreaterThan(nowInMillis);
        expect(response.body.message).toBe('You are not authorized to delete this hoax');
    })
